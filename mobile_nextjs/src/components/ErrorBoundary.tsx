        ...
        if (this.state.hasError) {
            ...
            return (
                <div className="flex flex-col items-center justify-center min-h-[300px] px-6 text-center">
                    ...
                    <p className="text-sm font-semibold text-foreground mb-1">Bir şeyler ters gitti</p>
                    <p className="text-xs text-muted-foreground mb-4 max-w-xs">
                        {this.state.error?.message || 'Beklenmeyen bir hata oluştu'}
                    </p>
                    <button
                        onClick={this.handleRetry}
                        className="flex items-center gap-2 text-xs bg-primary text-primary-foreground px-4 py-2 rounded-lg hover:bg-primary/90 transition-colors"
                    >

...
export function StatsTab() {
    ...
    if (!stats) return (
        <div className="p-8 text-center text-muted-foreground text-xs">
            ...
            <button onClick={load} className="block mx-auto mt-2 text-primary text-xs">Tekrar Dene</button>
        </div>
    );
    ...
    return (
        <div className="p-4 space-y-4 pb-8">
            ...
            <div className="grid grid-cols-3 gap-2">
                <StatCard icon={<AlertTriangle className="w-3.5 h-3.5 text-severity-fatal" />}
                    label="Toplam" value={String(total)} />
                <StatCard icon={<Clock className="w-3.5 h-3.5 text-severity-critical" />}
                    label="Bekleyen" value={String(stats.pending_logs)} />
                <StatCard icon={<Activity className="w-3.5 h-3.5 text-primary" />}
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                <h3 className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-3">Severity Dağılımı</h3>
                        ...
                        {(['fatal', 'critical', 'warn'] as Severity[]).map((s) => {
                            ...
                            return (
                                <div key={s} className="flex items-center gap-2">
                                    ...
                                    <span className="text-[11px] text-foreground flex-1">{SEVERITY_CONFIG[s].label}</span>
                                    <span className="text-[11px] font-bold text-foreground">{sevCounts[s]}</span>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                className="bg-card/40 rounded-lg p-4 border border-border">
                <h3 className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-3">Kategori Dağılımı</h3>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                className="bg-card/40 rounded-lg p-4 border border-border">
                <h3 className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-3">Son 24 Saat</h3>
                <div className="flex items-end gap-[2px] h-16">
                    {hourBuckets.map((count, i) => (
                        <motion.div key={i}
                            initial={{ height: 0 }} animate={{ height: `${(count / maxBucket) * 100}%` }}
                            transition={{ duration: 0.4, delay: i * 0.02 }}
                            className="flex-1 bg-primary/60 rounded-t-sm min-h-[2px] hover:bg-primary transition-colors" />
                    ))}
                </div>
                <div className="flex justify-between mt-1">
                    <span className="text-[8px] text-muted-foreground">24s önce</span>
                    <span className="text-[8px] text-muted-foreground">şimdi</span>
                </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                className="bg-card/40 rounded-lg p-4 border border-border">
                <h3 className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-3">Son Fatal & Critical</h3>
                <div className="space-y-2">
                    {alerts
                        ...
                        .map((a) => {
                            ...
                            return (
                                <div key={a.id} className="flex items-center gap-2 text-[11px]">
                                    ...
                                    <span className="flex-1 truncate text-foreground">{a.title || a.summary}</span>
                                    <span className="text-muted-foreground shrink-0">{formatDate(a.created_at || a.timestamp)}</span>
                                </div>
                            );
                        })}
                    {alerts.filter((a) => { const s = mapSeverity(a.severity); return s === 'fatal' || s === 'critical'; }).length === 0 && (
                        <p className="text-[11px] text-muted-foreground text-center py-2">Henüz yok</p>
                    )}
                </div>
            </motion.div>
        </div>
...
function StatCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
    return (
        <div className="bg-card/40 rounded-lg p-3 border border-border text-center">
...
function DonutChart({ data, colors, total }: { data: Record<Severity, number>; colors: Record<Severity, string>; total: number }) {
    ...
    return (
        <svg viewBox="0 0 96 96" className="w-full h-full -rotate-90">
            {/* Background ring */}
            <circle cx="48" cy="48" r={radius} fill="none" stroke="rgb(var(--muted))" strokeWidth={stroke} />
            {/* Data segments */}
            {segments.map((s) => {
            ...
            <text x="48" y="48" textAnchor="middle" dominantBaseline="central"
                className="fill-foreground font-black text-lg rotate-90 origin-center" fontSize="18">
                {total}
            </text>
        </svg>
    );
